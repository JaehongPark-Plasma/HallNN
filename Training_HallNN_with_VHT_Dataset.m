...
% Load training dataset "only virtual Hall thruster dataset"
clc;clear;
load("Data/DATA_VHT_in.mat");
load("Data/DATA_VHT_out.mat");

% Logarithmic transformation of input and output
input_c = log(rinput); % Logarithmic transformation
output_c = log(routput); % Logarithmic transformation
...
disp(['MSE Result :: train: ' ,num2str(Train_mean(end)),' | test:' ,num2str(Test_mean(end))]);
...
saveas(gcf,['results/HallNN_VHTver_perf','.png']);
